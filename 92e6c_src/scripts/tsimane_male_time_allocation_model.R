...
## The model is run separately on male and females. Restrict to males here:
a <- a[a$male == "1",]
...
## tsimane male model has random effects for individuals (i), community (c), and month (m), and all fixed effects
## Fixed effects include age, age squared, time of day, and time of day squared

#######################################################################################################

dat_list_tsimane_male <- list(
  K = K,
  N = N,
  N_id = N_id,
  N_com = N_com,
...
n_chains_tsimane_male <- 4 
init_tsimane_male <- list()
for ( i in 1:n_chains_tsimane_male ) init_tsimane_male[[i]] <- start_tsimane_male

mfit_tsimane_male <- stan( model_code=model_code_tsimane_male , data=dat_list_tsimane_male , chains=n_chains_tsimane_male , cores= n_chains_tsimane_male , warmup=1000, iter=2000, init=init_tsimane_male , control = list(adapt_delta = 0.95))

save.image("rstan_male_final")


# do some processing on cluster (crashes weak laptop)
# these outputs get used in subsequent analysis file
post <- extract.samples(mfit_tsimane_male)
a_male <- a
save(post, file="post_male_final")
save(a_male, file= "a_male_final")
