...
mfit_hadza_male <- stan( model_code=model_code_icmF , data=dat_list_icmF , chains=n_chains_icmF , cores= n_chains_icmF , warmup=500, iter=1000, init=init_icmF , control = list(adapt_delta = 0.95))

save.image("rstan_hadza_male_5_29_2020.RData")


# do some processing on cluster (crashes weak laptop)
# these outputs get used in subsequent analysis file
post_male <- extract.samples(mfit_hadza_male)
a_male <- a
save(post_male, file="post_male_hadza_final_5_29_2020.RData")
save(a_male, file= "a_male_hadza_final_5_29_2020.RData")

#diagnostics 
check_hmc_diagnostics(mfit_hadza_male)